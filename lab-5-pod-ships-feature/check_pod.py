...
Stdlib only, offline, deterministic: no pytest, no YAML library, no network.
    ...
    python3 check_pod.py                       # check Lab 5's required pod
    python3 check_pod.py --scope both          # search ~/.claude/agents/ too (the default here)
    python3 check_pod.py --require contract-reviewer
...
# Lab 5 authors two roles and carries three forward from Lab 4: `implementer` (migrates WM-114,
# then builds WM-118), `test-author` (originates WM-118's tests, its only appearance all day
# where it does that job rather than repairing a stale one), and `contract-reviewer` (reviews
# both). All three are copied in from Lab 4's .claude/agents/, see README.md Part 2.
# `impact-mapper` is authored at *personal* scope (~/.claude/agents/), so the default search
# scope here is "both" rather than "project".
DEFAULT_POD = ("impact-mapper", "release-scribe", "implementer", "test-author", "contract-reviewer")
...
def main() -> int:
    ...
    parser.add_argument("--require", default=",".join(DEFAULT_POD),
                        help="comma-separated role names that must exist (default: Lab 5's pod)")
    parser.add_argument("--scope", default="both", choices=("project", "personal", "both"),
                        help="where to look for agent definitions (default: both)")
