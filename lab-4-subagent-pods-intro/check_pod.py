...
Stdlib only, offline, deterministic: no pytest, no YAML library, no network.
    ...
    python3 check_pod.py                       # check Lab 4's required pod
    python3 check_pod.py --require test-author,implementer,contract-reviewer
...
# Three of these ship with the lab, already written, as worked examples. The fourth
# (`reference-data-steward`) is the one you author in Part 3, so a fresh run of this script
# shows three [ok] lines and one [FAIL], which is exactly the shape of your remaining work.
DEFAULT_POD = ("test-author", "implementer", "contract-reviewer", "reference-data-steward")
...
def main() -> int:
    ...
    parser.add_argument("--require", default=",".join(DEFAULT_POD),
                        help="comma-separated role names that must exist (default: Lab 4's pod)")
